...
int main(int argc, char *argv[])
{
    ...
    while (fscanf(infile, "%s", name) == 1) // for each line.
    {
        node *cursor = root;

        for (int i = 0, n = strlen(name); i < n; i++)
        {
            int index = tolower(name[i]) - 'a'; // get the index for each letter in the name
            if (cursor->children[index] == NULL) // if the letter the previous node == NULL, it means not used yet, so need
            // to allocate another node for it to point to.
            {
                // Make node
                ...
                cursor->children[index] = new; // point letter to new node since it is not the end of the name yet.
            }
...
bool check(char* word)
{
    ...
    for (int i = 0, len = strlen(word); i < len; i++)
    {
        ...
        if (iter->children[index] != NULL && iter->is_word == true && i == len - 1)
        {
            return true;
        }
        if (iter->children[index] != NULL) // if not the end of the word and the letter has a next node, keep going
        {
    ...
    return iter->is_word; // note: the is_word you are checking is in the node that the last letter's index is pointing to.
}
