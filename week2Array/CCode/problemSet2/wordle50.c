...
int check_word(string guess, int wordsize, int status[], string choice)
{
    ...
    // iterate over each letter of the guess
        // iterate over each letter of the choice
            // compare the current guess letter to the current choice letter
                // if they're the same position in the word, score EXACT points (green) and break so you don't compare
                    // that letter further
    ...
    for (int i = 0; i < wordsize; i++)
    {
        for (int j = 0; j < wordsize; j++)
        {
            ...
            else if (guess[i] == choice[j]) // && status[j] == WRONG)
            {
                // Must check if the status of the choice index being checked is `WRONG,` meaning that it has not been set yet.
                // Without the check, for choice word `charm,` guessing `ccccc,` would result in [1] = EXACT and the rest CLOSE
                // In this assignment, don't want it. They want it to show all `CLOSE`
                status[i] = CLOSE;
                score += 1;
            }
        }
...
Results for cs50/problems/2023/x/wordle generated by check50 v3.3.8
:) wordle.c exists
:) wordle.c compiles
